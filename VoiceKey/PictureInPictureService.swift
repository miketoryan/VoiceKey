...
final class PictureInPictureService: NSObject,
    ...
    func attachPreview(to host: UIView) {
        ...
        renderStatus(
            title: isActive ? "VoiceKey Ready" : "VoiceKey",
            subtitle: isActive
                ? "Skip app switching is active"
                : "Enable Skip App Switching below"
        )
    }
    ...
    enum PiPError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .unavailable:
                "VoiceKey could not create Picture in Picture."
            case .notReady:
                "Picture in Picture is not ready yet. Keep VoiceKey visible and try again."
            }
        }
    }
}
